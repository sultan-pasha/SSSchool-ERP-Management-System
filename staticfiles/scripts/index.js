...
function navHide(){
    ...
    if(cscro > 0 && lastscroll <= cscro && window.screen.availHeight > 600){
        lastscroll = cscro;
        navbar.style.cssText = 'height: 42px; position: fixed; top:0; left: 0; right: 0; background-color:#198754!important; padding:0';
        img.style.cssText = 'width: 50px;';
        body.style.cssText="padding-top:65px; overflowY:scroll";
        navcollapse.classList.remove("show");
    }
    else if(cscro > 0 && lastscroll <= cscro && window.screen.availHeight < 600 && navcollapse.classList[2]=="show"){
        lastscroll = cscro;
        navbar.style.cssText = 'height: 100vh; width=100vh; position: fixed; top:0; overflow:scroll;';
        body.style.cssText="padding-top:65px; overflow:hidden";
    }
    ...
    else if(cscro > 0 && lastscroll-180 >= cscro && window.screen.availHeight > 600){
        lastscroll=cscro
